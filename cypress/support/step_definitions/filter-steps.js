...
Then(
  "I should be presented with the cars that meets the expectations {word} {word} {word}",
  (nadwozie, marka, max) => {
    cy.get("[class='ooa-17z33ro'] [type='text']").should(
      "have.value",
      nadwozie
    );
    cy.get("[class='ooa-1obpyoo'][type='text']")
      .invoke("attr", "placeholder")
      .should("contain", marka);
    cy.get(
      "div:nth-of-type(5) > div > div:nth-of-type(2) input[role='combobox']"
    )
      .invoke("attr", "value")
      .should("contain", addSpacesInNumber(max));

    cy.get("[data-testid='listing-ad']").then(($cars) => {
      carsCount = Cypress.$($cars).length;
      cy.log(carsCount);
    });
    cy.get(".ooa-x3g7qd-Text")
      .invoke("text")
      .then((text1) => {
        ...
        cy.get("body").then(($body) => {
          if ($body.find('[data-testid="pagination-list"]').length) {
            cy.get('[data-testid="pagination-list"]').should(
              "contain",
              Math.ceil(number / carsCount)
            );
          } else {
        ...
        cy.get("[data-testid='listing-ad'] .e1b25f6f8").each(
          ($el, index, list) => {
            cy.wrap($el)
              .invoke("text")
              .then((text2) => {
...
const addSpacesInNumber = (number) =>
